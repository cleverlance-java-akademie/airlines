...
public class AirportClient implements IAirportClient
{
    ...
    @PostConstruct /*po zavolani konstruktoru teto tridy se zavola tato metoda*/
    ...
    private List<Destination> convertToDestinations(final AirportListGen airportList)
    {
        return airportList.stream().map(item -> airportMapper.convertToDestination(item)).collect(Collectors.toList());
    }

//    private Destination convertToDestination(final AirportGen airport)
//    {
//        return Destination.builder().code(airport.getCode()).name(airport.getName()).build();
//    }
}
